...
PINDEL_BIN="/home/groups/clinical/RichardsLab/bin/pindel"
PINDEL2VCF_BIN="/home/groups/clinical/RichardsLab/bin/pindel2vcf"
...
def pindel(reference, configFile, args, tempDir, chrome=None):
    ...
    cmd = "%s -f %s -i %s -o %s " %(PINDEL_BIN, reference, configFile, pindel_file_base )
...
def pindel2vcf(inputFastaFile, refName, pindel_file, vcf_file):
    ...
    cmd = "%s -p %s -r %s -R %s -d %s -v %s" % (PINDEL2VCF_BIN, pindel_file, inputFastaFile, refName, date, vcf_file)
    return cmd
...
def __main__():
    ...
    parser.add_argument('--number_of_procs', dest='procs', type=int, default=16)
